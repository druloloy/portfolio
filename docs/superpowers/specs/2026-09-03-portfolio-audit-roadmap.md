...
| ID | Sev | Finding | Location |
|---|---|---|---|
| F1 | P0 | `export const dynamic = 'force-dynamic'` on all three route files, plus `cache: 'no-store'` on every fetch — while `revalidateTag` hooks and a `/next/revalidate` endpoint sit fully built. **The entire caching layer is dead code and every request hits Postgres.** | 3 route files, `_api/fetchDoc.ts:48` |
